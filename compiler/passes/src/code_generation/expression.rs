...
impl CodeGeneratingVisitor<'_> {
    pub fn visit_expression(&mut self, input: &Expression) -> (String, String) {
        ...
        let is_pure = if let Expression::AssociatedFunction(ass_fun_expr) = input
            && let Some(core_fn) = CoreFunction::try_from(ass_fun_expr).ok()
        {
            core_fn.is_pure()
        } else if let Expression::Call(..) = input {
            // We may be calling an impure function
            false
        } else {
            true
        };
