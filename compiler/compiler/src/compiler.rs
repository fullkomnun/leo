...
impl Compiler {
    ...
    pub fn compile(
        ...
        let parsed_program = self.state.ast.ast.clone();
    ...
    pub fn add_import_stubs(&mut self) -> Result<()> {
        ...
        while let Some(import) = to_explore.pop() {
            ...
            if let Some(stub) = self.import_stubs.get(&import) {
                for new_import_name in stub.imports() {
                    if !explored.contains(new_import_name) {
                        to_explore.push(*new_import_name);
                    }
                }
            } else {
